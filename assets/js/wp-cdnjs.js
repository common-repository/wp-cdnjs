...
jQuery(document).ready(function(jQuery) {
            ...
            for(var j = 0; j < data.results.length; j++) {
              for(var i = 0; i < usedPlugins.length; i++) {
                if(usedPlugins[i] === data.results[j].name) {
                  if (debug()) console.log('remoteAjaxAutoComplete-data.results[j]:');
  ...
  function formatLibraryResults(data) {
    ...
    if (debug()) console.log('data.results=',data.results);
    if (debug()) console.log('data.version=',data.version);
    if (debug()) console.log(data);
    // 20200319: looks like cdnjs does not return the number of versions available anymore:
    ...
    return '<div id="opt-' + data.filename + '" class="cdnjs-selection" title="' + data.description + '">'+data.name+' '+data.version+' '+data.description+'</div>';
  }
...
function onAssetChange() {
  jQuery('.select2-assets').not('.select2-offscreen').each(function(i, obj) {
    ...
    objID.select2({
      ...
      ajax:            {
        // no assets no more i said!
        // url:      '//api.cdnjs.com/libraries?fields=assets',
        url:      '//api.cdnjs.com/libraries?fields=name,filename,version,description',
        dataType: 'json',
        data:     function(term, page) {
          return {
        ...
        results:  function(data, page) {
          ...
          //for (j = 0; j < data.results[0].assets.length; j++) {
            ...
            asset = data.results[0]; //Object { version: "1.7.1", files: Array[react-popper.js,react-popper.min.js] }
            if (selectedVersion == asset.version) {
              //for(i = 0; i < asset.files.length; i++) {
                ...
                assetToCheck = (!jQuery('#cdnjs_settings_avoid_minified').prop("checked")) ? asset.latest : asset.latest.replace('.min', '');
    ...
    objID.select2({
      ...
      ajax:            {
        ...
        results:  function(data, page) {
          ...
          var assets = [data.results[0]]; // Object [{ version: "1.7.1", files: Array[react-popper.js,react-popper.min.js] },{..}]
...
function addLibraryRow(data, location) {
  ...
  // 20200319: we no longer have assets:
  // if (debug()) console.log('addLibraryRow-data.assets: ',data.assets);  //Object { version: "0.7.4", files: Array[2] }
  ...
  if (!jQuery('#cdnjs_settings_avoid_minified').prop("checked")) {
    // there is only one case: remove .min from default_asset = data.filename
    default_asset == default_asset.replace('.min', '');

/*
    // 20200319: this is gone with the wind
...
*/
  ...
  row += '<td class="wp-cdnjs_choose_version">'+data.version+'<input type="hidden" id="' + nameID + '-choose_version" data-plugin-id="' + nameID + '" data-plugin-name="' + data.name + '" data-version="' + data.version + '" class="select2-version"></td>';
...
function addAssetRow(data, location) {
  ...
  var assetId = cleanName(location);
